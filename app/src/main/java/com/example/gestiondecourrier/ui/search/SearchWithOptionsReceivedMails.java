...
public class SearchWithOptionsReceivedMails extends Filter {

    List<MailResponse> originalList;
    List<MailResponse> filteredList;
    RecyclerViewHome recyclerViewHome;
    private String category="",structure="",entryDate="",departureDate="";



    public SearchWithOptionsReceivedMails(List<MailResponse> originalList, RecyclerViewHome recyclerViewHome) {
        ...
        if (charSequence.length()==0)
            ...
            for ( MailResponse item : originalList) {
                ...
                if (item.getInternalReference().toLowerCase().contains(filterPattern)) {
                    if (item.getReceivedCategory()!=null && !category.isEmpty() && !item.getReceivedCategory().trim().equalsIgnoreCase(category.trim())) {
                        match = false;
                    }
                    if (!structure.isEmpty() && !item.getSender().trim().equalsIgnoreCase(structure.trim())) {
                        match = false;
                    }

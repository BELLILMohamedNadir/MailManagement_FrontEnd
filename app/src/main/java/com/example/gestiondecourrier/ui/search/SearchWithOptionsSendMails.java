...
public class SearchWithOptionsSendMails extends Filter {
    ...
    public SearchWithOptionsSendMails(List<MailResponse> originalList, RecyclerViewHome recyclerViewHome) {
        ...
        if (charSequence.length()==0)
            ...
            for ( MailResponse item : originalList) {
                ...
                if (item.getInternalReference().toLowerCase().contains(filterPattern)) {
                    ...
                    if (!structure.isEmpty() && !item.getForStructure().trim().equalsIgnoreCase(structure.trim())) {
                        match = false;
                    }
